...
- [ ] T066 [PR-5] Run full 50-scenario benchmark **manually** (real LLM cost ~¥30-50; budget is CNY now — Volcano Ark bills in CNY, no conversion): `python scripts/run_benchmark.py --scenarios benchmarks/scenarios/ --budget 150`. Open `benchmarks/results/{run_id}/summary.json`; assert `v2.latency_ms.p95 ≤ 90000` (SC-008), `v2.resolution_rate ≥ 0.70` (SC-009), `security_subset.v2_false_negative_count == 0` (SC-013, non-negotiable). Attach full summary.json to PR-5 description.
  - **As-built (run `20260605-045858-598f867`, branch `005-pr5-glm-tracer-fallback`)**: kept `[ ]` — **2 of 3 SCs met**. SC-009 ✅ `resolution_rate=0.98`; SC-013 ✅ `v2_false_negative_count=0`; **SC-008 ❌ `p95≈99.9s` (>90s)**. The real run uncovered + fixed 6 bugs (all Test-First on the branch): (1) `glm_client` crashed when `llmops_dashboard` absent → nullcontext fallback; (2) SecurityReviewer prompt judged "destructiveness" not Principle-V categories → realigned; (3) 4 security scenarios (004/030/031/033) were mis-labelled HIGH_RISK on the *incident* class → relabelled SAFE (fix artifact is judged, not the incident); (4) GLM semantic review unreliably SAFE'd secret-touching fixes → deterministic secret/credential HIGH_RISK override (032/034/035); (5) `3×30s` retry amplified timeout latency → `2×45s`; (6) a GLM timeout on the security review produced no verdict (SC-013 regression) → fail-safe HIGH_RISK on LLM failure. **SC-008 is structural, not a bug**: 3 INFRA scenarios (020/018/025) genuinely run 110-115s (no timeouts) on the 5-call sequential reasoning pipeline; ≤90s needs a faster security model or parallel agents — deferred as a perf follow-up (developer decision).
