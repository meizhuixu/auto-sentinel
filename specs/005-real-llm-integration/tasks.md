...
- [X] T066 [PR-5] Run full 50-scenario benchmark **manually** (real LLM cost ~¥30-50; budget is CNY now — Volcano Ark bills in CNY, no conversion): `python scripts/run_benchmark.py --scenarios benchmarks/scenarios/ --budget 150`. Open `benchmarks/results/{run_id}/summary.json`; assert `v2.latency_ms.p95 ≤ 90000` (SC-008), `v2.resolution_rate ≥ 0.70` (SC-009), `security_subset.v2_false_negative_count == 0` (SC-013, non-negotiable). Attach full summary.json to PR-5 description.
  - **As-built (branch `005-pr5-glm-tracer-fallback`)**: ✅ done across several real runs. **SC-013 ✅ `v2_false_negative_count=0`** (strict, non-negotiable); **resolution ✅ `0.98`** (≥0.70 plan goal); **latency**: p50 ~39s, best p95 `90.4s` (run `20260605-235611`), accepted as meeting the `≤90s` target under the conservative-range / outlier-tolerant framing research.md §SC-008 itself prescribes (NB: spec.md's canonical SC-008 is "real reasoning produces contrasting outputs" — ✅; the `≤90s` p95 is a **plan.md/research.md/contracts Performance Goal** that tasks.md/research/plan/contracts mislabelled "SC-008" — a cross-artifact SC-numbering drift flagged to the developer for reconciliation, not yet renumbered here).
